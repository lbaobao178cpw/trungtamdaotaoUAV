...
import { AlertTriangle, Loader2, Box, AlertCircle } from "lucide-react"; // 1. IMPORT ICON LUCIDE
import MediaSelector from "../mediaSelector/MediaSelector";
import "./Model3DManager.css";

// === WEBGL CONTEXT CHECKER ===
class CanvasWrapper extends React.Component {
  constructor(props) {
    super(props);
    this.state = { canUseWebGL: true, error: null };
  }
  ...
  componentDidMount() {
    ...
    try {
      ...
      if (!gl) {
        this.setState({ canUseWebGL: false, error: "WebGL không khả dụng trên trình duyệt này" });
      }
    } catch (e) {
      this.setState({ canUseWebGL: false, error: e.message });
    }
  }

  render() {
    const { canUseWebGL, error } = this.state;
    if (!canUseWebGL) {
      return (
        <div style={{
          width: "100%",
          ...
          <div style={{ fontSize: "14px", color: "#666", maxWidth: "400px", lineHeight: "1.5" }}>
            WebGL không khả dụng trong môi trường này. Điều này có thể do:<br/>
            - GPU bị vô hiệu hóa (Sandboxed mode)<br/>
            - Trình duyệt không hỗ trợ WebGL<br/>
            - Cài đặt bảo mật chặn WebGL
          </div>
        </div>
      );
    }
