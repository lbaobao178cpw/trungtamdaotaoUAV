  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: '18px', fontWeight: '600', color: '#fff' }}>
        Không thể tải bản đồ 3D
      </div>
      <div style={{ fontSize: '14px', color: '#ccc', maxWidth: '400px', lineHeight: '1.6' }}>
        WebGL không khả dụng trên trình duyệt này. Các tính năng 3D không thể hiển thị.
      </div>
    </div>
  );
}

// === CANVAS WRAPPER WITH WEBGL CHECK ===
class CanvasWrapper extends React.Component {
  constructor(props) {
    super(props);
    this.state = { canUseWebGL: null, error: null };
  }
  ...
  componentDidMount() {
    try {
      ...
      if (!gl) {
        this.setState({ canUseWebGL: false });
      } else {
        this.setState({ canUseWebGL: true });
      }
    } catch (e) {
  ...
  render() {
    ...
    if (canUseWebGL === null) {
      return <div style={{ width: '100%', height: '100%', backgroundColor: '#222222' }} />;
    }
