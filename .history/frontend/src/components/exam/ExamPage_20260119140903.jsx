  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch FAQs từ API
        const faqResponse = await fetch("http://localhost:5000/api/faqs");
        if (faqResponse.ok) {
                  ...
                  {faqDataList.map((faq, idx) => (
                    <div key={idx} className="faq-item">
                      <div className="faq-question" onClick={() => toggleFAQ(idx)}>
                        ...
                        <ChevronDown className={`w-5 h-5 text-primary transition-all duration-400 ${openFAQ === idx ? 'rotate-180' : ''}`} />
                      </div>
                      {openFAQ === idx && (
