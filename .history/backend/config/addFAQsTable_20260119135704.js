...
const createFAQsTable = async () => {
  try {
    console.log('🔧 Kiểm tra bảng faqs...');
    ...
    console.log('✅ Bảng faqs đã được kiểm tra/tạo thành công!');

    // Insert sample data if table is empty
    const [rows] = await db.execute('SELECT COUNT(*) AS count FROM faqs');
    if (rows[0].count === 0) {
      console.log('📝 Thêm dữ liệu mẫu vào bảng faqs...');
      ...
      console.log('✅ Đã thêm 8 FAQ mẫu!');
    }

  } catch (error) {
    console.error('❌ Lỗi tạo bảng faqs:', error.message);
  } finally {
    await db.end();
  }
};
