...
// 1. Lấy danh sách thẩm quyền - SIMPLE TEST VERSION
router.get("/authorities", async (req, res) => {
  console.log("=== API /authorities TEST VERSION ===");

  try {
    // Bỏ qua tất cả filter, chỉ lấy dữ liệu cơ bản
    ...
    // Đếm tổng
    const [countRows] = await pool.execute("SELECT COUNT(*) as total FROM authorities");
    const total = countRows[0].total;
    const totalPages = Math.ceil(total / limit);

    console.log(`Success: Trả về ${rows.length} authorities`);
    ...
    console.error("Lỗi test API authorities:", err);
    res.status(500).json({
      success: false,
