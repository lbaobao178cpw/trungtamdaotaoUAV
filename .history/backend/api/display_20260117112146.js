...
router.post("/legal-documents", async (req, res) => {
  try {
    ...
    // Normalize status - chỉ cho phép các giá trị hợp lệ
    const validStatus = ['active', 'expired', 'amended', 'draft'];
...
router.put("/legal-documents/:id", async (req, res) => {
  try {
    ...
    // Normalize is_featured thành 0 hoặc 1
    ...
    if (effective_date !== undefined) {
      ...
      values.push(effective_date || null);
    }
    ...
    // Always update status since we normalized it
    ...
    console.log("UPDATE SQL - Status value:", String(status).trim(), "Length:", String(status).trim().length);
...
// Check cấu trúc bảng legal_documents
router.get("/check-legal-documents-structure", async (req, res) => {
  try {
    const [columns] = await pool.execute(
      "SELECT COLUMN_NAME, COLUMN_TYPE, IS_NULLABLE, COLUMN_KEY FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'legal_documents' AND TABLE_SCHEMA = DATABASE()"
    );
    ...
    console.error("Lỗi check structure:", err);
    res.status(500).json({ success: false, message: err.message });
  }
});
