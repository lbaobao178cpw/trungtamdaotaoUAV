...
router.get("/authorities", async (req, res) => {
  try {
    ...
    // SỬA: Không dùng alias 'a.' trong WHERE clause
    ...
    // Lấy dữ liệu phân trang - SỬA
    const [rows] = await pool.execute(
      `SELECT a.*, 
              parent.name as parent_name 
       FROM authorities a
       LEFT JOIN authorities parent ON a.parent_id = parent.id
       ${whereClause.replace("is_active", "a.is_active")}  // SỬA: thêm alias 'a.'
...
// 1. Lấy danh sách biểu mẫu - FIXED
router.get("/forms", async (req, res) => {
  try {
    ...
    // SỬA: Query chính với tham số đúng thứ tự
    const query = `
      SELECT f.*, 
    ...
    // SỬA: Thêm limit và offset vào cuối mảng params
