...
// 1. Lấy danh sách thẩm quyền - FIXED
// ==========================================
// 6. API THẨM QUYỀN (AUTHORITIES) - FIXED
// ==========================================

// 1. Lấy danh sách thẩm quyền - ĐÃ SỬA
// 1. Lấy danh sách thẩm quyền - FIXED COMPLETELY
router.get("/authorities", async (req, res) => {
  console.log("API /authorities được gọi với query:", req.query);
  ...
  try {
    ...
    const countQuery = `SELECT COUNT(*) as total FROM authorities ${whereClause}`;
    ...
    // Thêm WHERE clause với alias đúng
    if (conditions.length > 0) {
      // Thay thế "is_active" bằng "a.is_active" trong WHERE clause
      const aliasWhereClause = whereClause.replace("is_active", "a.is_active");
      dataQuery += ` ${aliasWhereClause}`;
    }

    dataQuery += ` ORDER BY a.level, a.name LIMIT ? OFFSET ?`;

    console.log("Data query:", dataQuery);

    // Tạo mảng params cho query chính
    const queryParams = [...params, limitNum, offset];
    console.log("Query params:", queryParams);

    const [rows] = await pool.execute(dataQuery, queryParams);

    console.log(`Trả về ${rows.length} authorities`);
...
// ==========================================
// 7. API BIỂU MẪU (FORMS) - FIXED
// ==========================================

// 1. Lấy danh sách biểu mẫu - ĐÃ SỬA
router.get("/forms", async (req, res) => {
  try {
    ...
    if (search) {
      conditions.push("(f.title LIKE ? OR f.description LIKE ? OR f.form_code LIKE ?)");
      const searchTerm = `%${search}%`;
      params.push(searchTerm, searchTerm, searchTerm);
    }
    ...
    // Query chính - SỬA: đúng thứ tự params
    const mainQuery = `
      SELECT f.*, 
    ...
    // SỬA: Thêm limit và offset vào cuối
    const queryParams = [...params, limitNum, offset];
