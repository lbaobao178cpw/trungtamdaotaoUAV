...
router.get("/legal-documents", async (req, res) => {
  try {
    ...
    // Lấy dữ liệu phân trang - FIX: thêm limit và offset vào params
    ...
    const [rows] = await pool.execute(
      `SELECT * FROM legal_documents ${whereClause} 
       ...
       LIMIT ? OFFSET ?`,
      queryParams  // Sử dụng queryParams thay vì spread operator
    );
...
router.get("/forms", async (req, res) => {
  try {
    ...
    if (search) {
      whereClause += " AND (title LIKE ? OR description LIKE ? OR form_code LIKE ?)";
      const searchTerm = `%${search}%`;
      params.push(searchTerm, searchTerm, searchTerm);
    }
