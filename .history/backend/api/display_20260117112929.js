...
router.post("/legal-documents", async (req, res) => {
  try {
    ...
    const normalizedStatus = validStatus.includes(status) ? status : 'active';
...
router.put("/legal-documents/:id", async (req, res) => {
  try {
    ...
    console.log("DEBUG: Normalized status value:", status, "Length:", status.length, "Type:", typeof status);
    ...
    console.log("DEBUG: About to execute UPDATE with values:", {
      status_value: values.find((v, idx) => updateFields[idx]?.includes('status')),
      all_values: values.map((v, idx) => ({field: updateFields[idx], value: v}))
    });
